...
- [x] **合并队友最新提交：小学期周次对齐（2026-08-11）**：队友 tsingsesu 新提交 11ef1a5（基于已合并的 006a467）：默认学期起始日期 2026-09-07 → **2026-08-24（小学期）**，课表周次对齐小学期——app.py index 路由 `semester_start` 默认值 + index.html 两处 JS 兜底 `new Date(2026, 7, 24)`。fast-forward 零冲突合并（分支底子已在 main，fetch 后直接 ff）。验证：重启后首页 200、`semesterStart="2026-08-24"` 生效

- [x] **界面整理：首页工作台 + 设置页 Tab（2026-08-11）**：解决「功能多了平铺杂乱」。
  ① **首页工作台**：统计卡 grid 3 列→4 列（修复 4 卡 3+1 两行）；新增「功能入口」网格 8 个 .home-tile（feather 图标语义色底 + 名称 + 描述 + 数字徽标，>0 才渲染；app.py index 路由新增注入 materials_count/review_count，复用 len(models.list_materials()/list_review_plans())）；任务区（需要马上处理+进行中）包进 .home-cols 双列（alert 卡空时 :only-child 跨全列）；**课表 section 整块原子沉底，内部零改动**。② **设置页 4 Tab**（.settings-tabs 下划线式：智能体/同步/邮件提醒/数据），面板 hidden 属性控制（⚠️ 必须配 `[hidden]{display:none!important}` 守卫防 .card 规则覆盖）；main.js 新增 switchSettingsTab(name) + hash 记忆（#tab-sync 直达，history.replaceState 不产生历史项）；**8 处联动点**：backup/cx导入/logout reload 前 location.hash 带 tab、crawl 失败自动滚动先切 sync 再 setTimeout 60ms 滚动（display:none 上 scrollIntoView 是 no-op）、showCaptchaDialog 开头切 sync、email/crawlSync/cxStartSync 防御性切换。③ **侧边栏**「学习管理」拆「任务」（首页/DDL/日历/复习）+「资源」（资料/搜索/导出）。④ **两个级联大坑**：a) 窄屏 .sidebar 是 flex 子项，默认 min-width:auto 会被 10 项横向导航内容撑开、overflow-x:auto 失效 → 必须 `min-width:0`；b) **CSS 级联顺序**：新增的 .home-grid 桌面规则（repeat(4,1fr)）写在文件末尾，晚于 860px 断点内规则 → 窄屏 4 列覆盖 2 列生效、tile 被压成 82px 宽、badge 溢出 10px → 窄屏覆盖规则必须也放文件末尾（级联：同特异性后者胜）。验证：Playwright 48/48 全过（统计卡同行/8 tile 链接/区块顺序/课表回归 6 项/深色/tab 切换/hash 直达/failed 日志自动切 sync/侧边栏分组/全站 7 页 200/390+1024 溢出=0）

- [x] **移动端改造 + 首页去重（2026-08-11）**：解决用户反馈「侧边栏与首页功能重复 + 手机端效果不好」。
  ① **首页去重**：删掉 8 个 .home-tile 功能入口网格（它就是侧边栏导航的复制品）及全部 tile CSS；app.py index 路由删掉只为 tile 徽标注入的 materials_count/review_count。首页回归「概览」定位：统计卡 4 张 + 任务双列 + 课表，导航统一走侧边栏/底部栏。② **移动端（≤860px）改为 App 形态**：侧边栏不再是顶部横条（10 项挤一排横向滚动，手机体验差），改成**左侧抽屉**（fixed、translateX(-100%)、.open 滑入、z-index 100、height:100dvh 跟随 iOS 地址栏）+ **底部固定 Tab 栏**（首页/DDL/日历/资料/更多 5 项，更多打开抽屉；active 态用 request.endpoint 判断，`{% set more_active = endpoint not in (...)%}`；抽屉内 nav-group/主题按钮/本地说明全部恢复显示）。③ main.js 新增 initMobileNav：开关 toggle、点遮罩关闭、ESC 关闭、点抽屉内链接自动关闭、body.menu-open 锁滚动。④ **关键坑：z-index 层级**——遮罩 z-index 99 曾盖住底部栏 90，抽屉打开时「更多」点不到（Playwright click 报 "menu-mask intercepts pointer events" 才暴露）；修复=底部栏提为 101（常驻 UI 永远可交互）+ 抽屉底部 padding 64px 防底栏盖住主题按钮。⑤ 旧 860px 断点块里侧边栏横条规则整体删除，抽屉规则放文件末尾新移动端块（级联后者胜，与上次同款坑）。验证：Playwright 51/51 全过（去重/抽屉开合/遮罩/toggle 关闭/点链接跳转设置页/各页 tab 高亮/设置页 4 tab 回归/390 深浅色+1024 门禁溢出=0/全站 10 页 200/无 console error）

- [x] **移动端课表：单日视图（2026-08-11）**：解决用户反馈「手机端课表要左右划，不方便」。
  整周表格 min-width:760px 在 390px 上只能横滚，改为**窄屏默认显示「今天」的单日纵向卡片视图**：每行=左侧时间列（第X节+开始时间，主题色）+ 右侧课程卡（课程色底、白字：课名/教师/教室），按上/下午/晚上分组；考试小测显示为红色/紫色横条（与整周视图同色）；没课显示「这天没有课 🎉」。工具栏新增「单日/整周」切换按钮（#sched-day-toggle，桌面隐藏、窄屏显示），切回整周即原有横向表格（DDL 覆盖层等原功能不动）。JS 集成：dayMode + viewDate（真实日期），窄屏初始化 dayMode=true；单日模式下翻周按钮=viewDate±7 天（周几不变）、sched-today=回到今天（文案同步「回到本周/回到今天」）；renderDayView 内部同步 curWeek 并调 renderSchedule 保证两视图数据一致（切回整周不用重载）。**⚠️ 跨 script 块坑：escapeHtml 定义在下方 DDL 脚本里，单日视图在课表脚本内同步调用 → 跨块不提升会 ReferenceError，课表脚本内先定义一份（同名声明后者覆盖，功能一致无副作用）**；day-view 显隐用 style.display 切换（hidden 属性会被 [hidden]{display:none!important} 守卫卡死）。验证：Playwright 30/30（默认单日/今天日期/前后天/回到今天/翻周同步/整周切换往返/整周模式翻周回归/桌面表格+覆盖层回归/深浅色门禁 390+1024 溢出=0/无 console error）
